...
mtcars%>%select(everything())%>%select(num_range('x',1:3)|matches("^[[:alpha:]]"))
...
mtcars %>%                 
  group_by(gear) %>%        
  summarise(mean= mean(!is.na(mpg)), sd=sd(mpg), 
            median=median(mpg), iqr=IQR(mpg), min=min(mpg))
...
data2 <- data.frame(ID1 = 2:3, ID=2:3,                     
                    X2 = c("b1", "b2"),
                    stringsAsFactors = FALSE)
